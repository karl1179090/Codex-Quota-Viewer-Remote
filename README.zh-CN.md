...
>
> 1.3.8 更新：
> - 远端账号切换成功后，会对已同步成功的 SSH 目标自动执行高级设置里的 **修复远程主机** 历史元数据修复。
> - **同步当前本机配置** 改为只发送当前账号对应的 provider 配置给远端合并，不再把整份本机 `config.toml` 直接上传覆盖。
> - 同步当前本机账号到远端时，会保留远端自己的 personality、MCP server 和主机专属设置。
> - 当 ChatGPT token 的 `account_id` 相同但 JWT subject 不同时，仍能区分为不同账号；账号自定义显示名旁会显示来源 snapshot 邮箱。
> - 远端终止 Codex 后会清理 stale desktop SSH websocket socket；只改 provider 元数据时会保留远端 rollout 文件修改时间。
>
> 1.3.7 更新：
...
远端账号切换成功后，还会执行高级设置里的 **修复远程主机** 历史元数据修复，让远端
Session Manager 元数据跟随切换后的 provider。从第三方 Provider 模式切回普通
ChatGPT 账号时，本机和远端都会移除遗留的 `[model_providers.custom]` 配置段。
切换确认框中还可以选择终止每个选中远端主机上 SSH 登录用户拥有的全部 `codex`
进程；远端切换时这个清理选项默认开启。
...
- 使用 **同步当前本机配置** 时，只会同步当前账号的 auth 和 provider 配置，不会替换远端
  `config.toml` 中无关的主机专属设置。
